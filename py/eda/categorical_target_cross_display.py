...
class CategoricalTargetCrossDisplay:
    """
    ...
    def _flow_category(self, df, col, tar_col):
        ...
        print(f'---------- Categorical Variable({col}) vs Target Count----------')
        _cross_df = pd.crosstab(_df[col], columns=[_df[tar_col]], margins=True)
        ipy_display(_cross_df)

        print(f'---------- Categorical Variable({col}) vs Target Normalize----------')
        _cross_df = pd.crosstab(_df[col], columns=[_df[tar_col]], margins=True, normalize=True)
        ipy_display(_cross_df)
        print('\n' * 5)
    ...
    def _flow_numerical(self, df, col, tar_col):
        ...
        print(f'---------- Numerical Variable({col}) vs Target Count----------')
        _cross_df = pd.crosstab(_df[col], columns=[_df[tar_col]], margins=True)
        ipy_display(_cross_df)

        print(f'---------- Numerical Variable({col}) vs Target Normalize----------')
        _cross_df = pd.crosstab(_df[col], columns=[_df[tar_col]], margins=True, normalize=True)
        ipy_display(_cross_df)
        print('\n'*5)
